...
ifeq ($(HOST_OS),darwin)
  ...
  KERNEL_MAKE_FLAGS += HOSTCFLAGS="$(shell /usr/bin/pkg-config libcrypto libssl --keep-system-cflags --keep-system-libs --cflags-only-I --libs-only-L)"
  KERNEL_MAKE_FLAGS += HOSTLDFLAGS="$(shell /usr/bin/pkg-config libcrypto libssl --keep-system-cflags --keep-system-libs --cflags-only-I --libs-only-L)"
endif
