...
int android_set_bfq_ioprio(int pid, int class, int prio) {
    int fd = -1, rc = -1, cgroup_set = 1;
    char cgroup[22];
    ...
    } else if (class == 2 && prio != 0) {
        snprintf(cgroup, sizeof(cgroup), "/dev/bfqio/be-%d/tasks", prio);
    } else if (class == 3) {
        snprintf(cgroup, sizeof(cgroup), "/dev/bfqio/idle/tasks", prio);
    } else {
        cgroup_set = 0;
    }
...
#ifdef HAVE_GETTID
    // Assume pid 0 means get current thread id
    if (pid == 0) {
        pid = gettid();
    }
#endif

    // specialized itoa -- works for pid > 0
    char text[22];
