...
mount_extra() {
  for partition in $1; do
    ...
    if [ -e "${BLK_PATH}/${partition}${SLOT_SUFFIX}" ]; then
      eval "$(to_upper $partition)_BLOCK"="${BLK_PATH}/${partition}${SLOT_SUFFIX}"
      [ "$DYNAMIC_PARTITIONS" = "true" ] && block --setrw "$PRODUCT_BLOCK"
      mount -o rw "$(to_upper $partition)_BLOCK" "$mnt_point"
    fi
  done
}
