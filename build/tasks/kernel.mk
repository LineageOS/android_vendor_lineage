...
ifdef BOARD_USE_QCOM_MERGE_DTBS
...
endif # BOARD_USE_QCOM_MERGE_DTBS
...
ifdef BOARD_USE_QCOM_MERGE_DTBS
	mkdir -p $(DTB_OUT)/overlay
	mkdir -p $(DTB_OUT)/out
	$(foreach techpackdir, audio camera cvp display eva mmrm video, \
		$(ifneq "$(wildcard $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts/vendor/qcom/$(techpackdir))","", \
			mv $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts/vendor/qcom/$(techpackdir) $(DTB_OUT)/overlay/))
	PATH=$(abspath $(HOST_OUT_EXECUTABLES)):$${PATH} python3 $(BUILD_TOP)/vendor/lineage/build/tools/merge_dtbs.py $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts $(DTB_OUT)/overlay $(DTB_OUT)/out
