...
#
#   BOARD_FORCE_PREBUILT_DTBO          = Optional, use BOARD_PREBUILT_DTBOIMAGE even
#                                          if device tree sources are present
#
...
# $(1): source path (The value passed to M=)
# $(2): target to build (eg. defconfig, modules, dtbo.img)
define make-external-module-target
$(PATH_OVERRIDE) $(KERNEL_MAKE_CMD) $(KERNEL_MAKE_FLAGS) -C $(1) M=$(2) KERNEL_SRC=$(BUILD_TOP)/$(KERNEL_SRC) O=$(KERNEL_BUILD_OUT_PREFIX)$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) $(KERNEL_CROSS_COMPILE) $(KERNEL_CLANG_TRIPLE) $(KERNEL_CC) $(KERNEL_LD) $(3)
endef
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			rootdir=$$(pwd); \
			rpath=$$(python3 -c 'import os,sys;print(os.path.relpath(*(sys.argv[1:])))' $$rootdir $(KERNEL_SRC)); \
			$(foreach p, $(TARGET_EXTERNAL_MODULES),\
				$(shell pwd); \
				$(call make-external-module-target,$(p),$$rpath/$(p),BUILD_TOP=$$rootdir); \
				$(call make-external-module-target,$(p),$$rpath/$(p),BUILD_TOP=$$rootdir INSTALL_MOD_PATH=$(MODULES_INTERMEDIATES) INSTALL_MOD_STRIP=1 modules_install)); \
...
ifneq ($(BOARD_FORCE_PREBUILT_DTBO),true)
...
endif # BOARD_FORCE_PREBUILT_DTBO
