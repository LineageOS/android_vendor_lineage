...
define build-image-kernel-modules-lineage
    ...
    for MODULE in $(5); do \
        if [ -n "$$(find $(2)/lib/modules$(6) -type f -name $$MODULE)" ]; then \
            basename $$MODULE >> $(2)/lib/modules$(6)/modules.load; \
        else \
            echo "ERROR: $$MODULE was not found in the kernel modules intermediates dir, module load list must be corrected" 1>&2 && exit 1; \
        fi; \
    done
endef
