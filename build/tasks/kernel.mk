	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			$(if $(SYSTEM_KERNEL_MODULES),\
				$(if $(filter true,$(TARGET_AUTO_COLLECT_KERNEL_MODULE_DEPS)),\
					gki_kernel_modules_deps+=$$($(COLLECT_MODULE_DEPS_CMD) $(KERNEL_OUT) $(SYSTEM_KERNEL_MODULES));\
				) \
			...
			$(if $(BOOT_KERNEL_MODULES) $(BOOT_KERNEL_MODULES_FINDER),\
				...
				$(if $(filter true,$(TARGET_AUTO_COLLECT_KERNEL_MODULE_DEPS)),\
					boot_kernel_modules_deps+=$$($(COLLECT_MODULE_DEPS_CMD) $(KERNEL_OUT) $(BOOT_KERNEL_MODULES) $$boot_kernel_modules_finder_output);\
				) \
