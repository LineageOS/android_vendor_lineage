	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			$(if $(SYSTEM_KERNEL_MODULES),\
				$(if $(filter true,$(TARGET_AUTO_COLLECT_KERNEL_MODULE_DEPS)), \
					gki_kernel_modules_deps=$$($(COLLECT_MODULE_DEPS_CMD) $(MODULES_INTERMEDIATES) $(SYSTEM_KERNEL_MODULES) | tr '\n' ' '); \
				) \
			...
			$(if $(BOOT_KERNEL_MODULES)$(BOOT_KERNEL_MODULES_FINDER),\
				$(if $(BOOT_KERNEL_MODULES_FINDER), \
					boot_kernel_modules_finder_output=$$($(BOOT_KERNEL_MODULES_FINDER) $(MODULES_INTERMEDIATES) | tr '\n' ' '); \
				) \
				$(if $(filter true,$(TARGET_AUTO_COLLECT_KERNEL_MODULE_DEPS)), \
					boot_kernel_modules_deps=$$($(COLLECT_MODULE_DEPS_CMD) $(MODULES_INTERMEDIATES) $(BOOT_KERNEL_MODULES) $$boot_kernel_modules_finder_output | tr '\n' ' '); \
				) \
			...
			$(if $(RECOVERY_KERNEL_MODULES)$(RECOVERY_KERNEL_MODULES_FINDER),\
				$(if $(RECOVERY_KERNEL_MODULES_FINDER), \
					recovery_kernel_modules_finder_output=$$($(RECOVERY_KERNEL_MODULES_FINDER) $(MODULES_INTERMEDIATES) | tr '\n' ' '); \
				) \
				$(if $(filter true,$(TARGET_AUTO_COLLECT_KERNEL_MODULE_DEPS)), \
					recovery_kernel_modules_deps=$$($(COLLECT_MODULE_DEPS_CMD) $(MODULES_INTERMEDIATES) $(RECOVERY_KERNEL_MODULES) $$recovery_kernel_modules_finder_output | tr '\n' ' '); \
				) \
