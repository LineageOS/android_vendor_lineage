...
RECOVERY_KERNEL_CONFIG := $(RECOVERY_OUT)/.config
RECOVERY_KERNEL_RELEASE := $(RECOVERY_OUT)/include/config/kernel.release
...
TARGET_PREBUILT_INT_RECOVERY_KERNEL := $(RECOVERY_OUT)/arch/$(KERNEL_ARCH)/boot/$(BOARD_KERNEL_IMAGE_NAME)
...
$(call internal-make-kernel-target,$(RECOVERY_OUT),$(1))
endef
...
RECOVERY_KERNEL_MODULES_OUT := $(TARGET_RECOVERY_OUT)
RECOVERY_DEPMOD_STAGING_DIR := $(KERNEL_BUILD_OUT_PREFIX)$(call intermediates-dir-for,PACKAGING,depmod_recovery)
RECOVERY_MODULE_MOUNTPOINT := recovery
RECOVERY_MODULES_INTERMEDIATES := $(KERNEL_BUILD_OUT_PREFIX)$(call intermediates-dir-for,PACKAGING,recovery_kernel_modules)

$(RECOVERY_OUT):
	mkdir -p $(RECOVERY_OUT)

$(RECOVERY_KERNEL_ADDITIONAL_CONFIG_OUT): $(RECOVERY_OUT)
	$(hide) cmp -s $(KERNEL_ADDITIONAL_CONFIG_SRC) $@ || cp $(KERNEL_ADDITIONAL_CONFIG_SRC) $@;

$(RECOVERY_KERNEL_CONFIG): $(RECOVERY_KERNEL_DEFCONFIG_SRC) $(KERNEL_ADDITIONAL_CONFIG_OUT)
	@echo "Building Recovery Kernel Config"
	$(call make-recovery-kernel-target,VARIANT_DEFCONFIG=$(VARIANT_DEFCONFIG) SELINUX_DEFCONFIG=$(SELINUX_DEFCONFIG) $(RECOVERY_DEFCONFIG))
	$(hide) if [ ! -z "$(KERNEL_CONFIG_OVERRIDE)" ]; then \
			echo "Overriding kernel config with '$(KERNEL_CONFIG_OVERRIDE)'"; \
			echo $(KERNEL_CONFIG_OVERRIDE) >> $(RECOVERY_OUT)/.config; \
	...
	$(hide) if [ ! -z "$(KERNEL_ADDITIONAL_CONFIG)" ]; then \
			echo "Using additional config '$(KERNEL_ADDITIONAL_CONFIG)'"; \
			$(KERNEL_SRC)/scripts/kconfig/merge_config.sh -m -O $(RECOVERY_OUT) $(RECOVERY_OUT)/.config $(KERNEL_SRC)/arch/$(KERNEL_ARCH)/configs/$(KERNEL_ADDITIONAL_CONFIG); \
			$(call make-recovery-kernel-target,KCONFIG_ALLCONFIG=$(RECOVERY_OUT)/.config alldefconfig); \
		fi
...
$(TARGET_PREBUILT_INT_RECOVERY_KERNEL): $(RECOVERY_KERNEL_CONFIG) $(DEPMOD) $(DTC)
	...
	$(hide) if grep -q '=m' $(RECOVERY_KERNEL_CONFIG); then \
			echo "Building Kernel Modules"; \
			$(call make-recovery-kernel-target,modules) || exit "$$?"; \
			echo "Installing Kernel Modules"; \
			$(call make-recovery-kernel-target,INSTALL_MOD_PATH=$(RECOVERY_MODULES_INTERMEDIATES) INSTALL_MOD_STRIP=1 modules_install); \
			kernel_release=$$(cat $(RECOVERY_KERNEL_RELEASE)) \
			kernel_modules_dir=$(RECOVERY_MODULES_INTERMEDIATES)/lib/modules/$$kernel_release \
			$(foreach s, $(TARGET_MODULE_ALIASES),\
				$(eval p := $(subst :,$(space),$(s))) \
				; mv $$(find $$kernel_modules_dir -name $(word 1,$(p))) $$kernel_modules_dir/$(word 2,$(p))); \
			modules=$$(find $$kernel_modules_dir -type f -name '*.ko'); \
			($(call build-image-kernel-modules-lineage,$$modules,$(RECOVERY_KERNEL_MODULES_OUT),$(RECOVERY_MODULE_MOUNTPOINT)/,$(RECOVERY_DEPMOD_STAGING_DIR),$(BOARD_RECOVERY_KERNEL_MODULES_LOAD))); \
			$(if $(BOOT_KERNEL_MODULES),\
				vendor_boot_modules=$$(for m in $(BOOT_KERNEL_MODULES); do \
					p=$$(find $$kernel_modules_dir -type f -name $$m); \
					if [ -n "$$p" ]; then echo $$p; else echo "ERROR: $$m from BOOT_KERNEL_MODULES was not found" 1>&2 && exit 1; fi; \
