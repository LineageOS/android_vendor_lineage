...
ifeq "$(wildcard $(KERNEL_SRC) )" ""
    ...
    ifneq ($(TARGET_KERNEL_PLATFORM_TARGET),)
        ifeq "$(wildcard $(abspath $(BUILD_TOP)/$(TARGET_KERNEL_PLATFORM_PATH))/$(KERNEL_SRC) )" ""
            $(warning ***************************************************************)
            $(warning *                                                             *)
...
ifneq ($(TARGET_KERNEL_PLATFORM_TARGET),)
KERNEL_PLATFORM_PATH_RE := $(subst /,\/,$(TARGET_KERNEL_PLATFORM_PATH))
KERNEL_PATH := $(abspath $(BUILD_TOP)/$(TARGET_KERNEL_PLATFORM_PATH))
$(TARGET_PREBUILT_INT_KERNEL): $(DEPMOD) $(KERNEL_MODULES_PARTITION_FILE_LIST) $(SYSTEM_KERNEL_MODULES_PARTITION_FILE_LIST)
	...
	@mkdir -p $(KERNEL_OUT)
	$(hide) cd $(KERNEL_PATH) && python3 $(BUILD_TOP)/.repo/repo/repo manifest -o - -r |sed '/^  <project.*\/>$$/{/$(KERNEL_PLATFORM_PATH_RE)/!d;}' |sed '/^  <project/,/  <\/project>/{/$(KERNEL_PLATFORM_PATH_RE)/!d;}' |sed 's|$(TARGET_KERNEL_PLATFORM_PATH)/||' > $(abspath $(KERNEL_OUT))/manifest.xml
