...
ifeq "$(wildcard $(KERNEL_SRC) )" ""
    ...
    ifneq ($(TARGET_KERNEL_PLATFORM_TARGET),)
        ...
        TARGET_PREBUILT_KERNEL := $(KERNEL_OUT)/$(BOARD_KERNEL_IMAGE_NAME)
        TARGET_PREBUILT_KERNEL_HEADERS := $(KERNEL_OUT)/kernel-uapi-headers.tar.gz
...
$(KERNEL_BIN):
	rm -rf $(KERNEL_PATH)/bazel* $(KERNEL_PATH)/out
	cd $(KERNEL_PATH) && ./tools/bazel run --cpu=$(KERNEL_ARCH) //$(KERNEL_SRC):$(TARGET_KERNEL_PLATFORM_TARGET)_dist
	mkdir -p $(KERNEL_OUT)
	cp $(KERNEL_PATH)/out/$(TARGET_KERNEL_PLATFORM_TARGET)/dist/* $(KERNEL_OUT)/
endif
