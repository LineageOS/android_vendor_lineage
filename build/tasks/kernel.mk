...
GKI_SUFFIX := $(shell echo android$(PLATFORM_VERSION)-$(KERNEL_VERSION)"."$(KERNEL_PATCHLEVEL))
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC) $(PAHOLE)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			$(if $(BOARD_SYSTEM_KERNEL_MODULES),\
				...
				[ $$? -ne 0 ] && exit 1; \
				($(call build-image-kernel-modules-lineage,$$gki_modules,$(SYSTEM_KERNEL_MODULES_OUT),$(SYSTEM_KERNEL_MODULE_MOUNTPOINT)/,$(SYSTEM_KERNEL_DEPMOD_STAGING_DIR),$(BOARD_SYSTEM_KERNEL_MODULES),/$(GKI_SUFFIX))); \
