...
ifneq (,$(findstring gki,$(KERNEL_DEFCONFIG)))
    ifeq ($(shell [ -d $(KERNEL_SRC)/scripts/gki ] && \
        ...
        cmd := export $(CORE_MAKE_FLAGS) && ARCH=$(KERNEL_ARCH) $(KERNEL_CROSS_COMPILE) CC="$(CCACHE_BIN) clang" KERN_OUT=$(KERNEL_OUT) MAKE_PATH=$(shell dirname $(KERNEL_MAKE_CMD))/ TARGET_BUILD_VARIANT=user $(TARGET_KERNEL_SOURCE)/scripts/gki/generate_defconfig.sh $(KERNEL_DEFCONFIG)
        _x := $(shell $(cmd))
    endif
endif
