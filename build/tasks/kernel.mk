	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			$(if $(TARGET_KERNEL_EXT_MODULES),\
				...
				$(foreach p, $(TARGET_KERNEL_EXT_MODULES),\
					$$pwd; \
					$(call $(if $(filter $(word 2,$(subst :, ,$(p))),kbuild),make-external-module-target,make-kbuild-module-target),$(word 1,$(subst :, ,$(p))),$$rpath,) || exit "$$?";  \
					$(call $(if $(filter $(word 2,$(subst :, ,$(p))),kbuild),make-external-module-target,make-kbuild-module-target),$(word 1,$(subst :, ,$(p))),$$rpath,INSTALL_MOD_PATH=$(MODULES_INTERMEDIATES) INSTALL_MOD_STRIP=1 KERNEL_UAPI_HEADERS_DIR=$(KERNEL_OUT) modules_install)) || exit "$$?"; \
			) \
