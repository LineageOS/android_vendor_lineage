...
KERNEL_ADDITIONAL_CONFIG_SRC := $(KERNEL_SRC)/arch/$(KERNEL_ARCH)/configs/$(KERNEL_ADDITIONAL_CONFIG)
    ifeq ("$(wildcard $(KERNEL_ADDITIONAL_CONFIG_SRC))","")
        $(warning TARGET_KERNEL_ADDITIONAL_CONFIG '$(TARGET_KERNEL_ADDITIONAL_CONFIG)' does not exist)
...
$(call target-kernel-hook)

ifndef HAVE_CACHED_KERNEL
...
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_CACHE_DIR := $(KERNEL_CACHE)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_OBJECT_DIR := $(KERNEL_OUT)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_SOURCE_DIR := $(KERNEL_SRC)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_KERNEL_IMG := $(TARGET_PREBUILT_INT_KERNEL)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_MODULE_DIR := $(KERNEL_MODULES_OUT)
...
$(KERNEL_HEADERS_INSTALL_DEPS): PRIVATE_CACHE_DIR := $(KERNEL_CACHE)
$(KERNEL_HEADERS_INSTALL_DEPS): PRIVATE_OBJECT_DIR := $(KERNEL_OUT)
...
INSTALLED_KERNEL_HEADERS: PRIVATE_CACHE_DIR := $(KERNEL_CACHE)
INSTALLED_KERNEL_HEADERS: PRIVATE_OBJECT_DIR := $(KERNEL_OUT)
INSTALLED_KERNEL_HEADERS: $(KERNEL_HEADERS_INSTALL_DEPS)
	$(save-prebuilt-kernel-headers)

else # HAVE_CACHED_KERNEL

LOCAL_PREBUILT_KERNEL_BINARY := $(KERNEL_CACHE)/$(BOARD_KERNEL_IMAGE_NAME)

$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_CACHE_DIR := $(KERNEL_CACHE)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_OBJECT_DIR := $(KERNEL_OUT)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_KERNEL_IMG := $(TARGET_PREBUILT_INT_KERNEL)
$(TARGET_PREBUILT_INT_KERNEL): PRIVATE_MODULE_DIR := $(KERNEL_MODULES_OUT)
$(TARGET_PREBUILT_INT_KERNEL): $(LOCAL_PREBUILT_KERNEL_BINARY)
	$(load-prebuilt-kernel-binaries)

.PHONY: INSTALLED_KERNEL_HEADERS
INSTALLED_KERNEL_HEADERS: PRIVATE_CACHE_DIR := $(KERNEL_CACHE)
INSTALLED_KERNEL_HEADERS: PRIVATE_OBJECT_DIR := $(KERNEL_OUT)
INSTALLED_KERNEL_HEADERS:
	$(load-prebuilt-kernel-headers)

endif # HAVE_CACHED_KERNEL

# Dependencies on $(KERNEL_OUT)/usr are deprecated
$(KERNEL_HEADERS_INSTALL_DIR): INSTALLED_KERNEL_HEADERS
