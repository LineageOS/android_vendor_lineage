...
# $(7): partition image intermediate where to update file list
...
define build-image-kernel-modules-lineage
    ...
    if [ ! -z "$(7)" ]; then \
        echo lib/modules$(6)/modules.alias >> $($(7))/file_list.txt; \
        echo lib/modules$(6)/modules.dep >> $($(7))/file_list.txt; \
        if [ ! -z "$(5)" ]; then echo lib/modules$(6)/modules.load >> $($(7))/file_list.txt; fi; \
        echo lib/modules$(6)/modules.softdep >> $($(7))/file_list.txt; \
        for MODULE in $(1); do \
            BASENAME=$$(basename $$MODULE); \
            echo lib/modules$(6)/"$$BASENAME" >> $($(7))/file_list.txt; \
        done \
    fi;
endef
...
PARTITION_FILE_LISTS := $(vendor_dlkmimage_intermediates)/file_list.txt
...
PARTITION_FILE_LISTS := $(vendorimage_intermediates)/file_list.txt
...
PARTITION_FILE_LISTS += $(system_dlkmimage_intermediates)/file_list.txt
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC) $(PARTITION_FILE_LISTS)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			$(if $(SYSTEM_KERNEL_MODULES),\
				...
				[ $$? -ne 0 ] && exit 1; \
				($(call build-image-kernel-modules-lineage,$$gki_modules,$(SYSTEM_KERNEL_MODULES_OUT),$(SYSTEM_KERNEL_MODULE_MOUNTPOINT)/,$(SYSTEM_KERNEL_DEPMOD_STAGING_DIR),$(BOARD_SYSTEM_KERNEL_MODULES_LOAD),/$(GKI_SUFFIX),$(SYSTEM_KERNEL_MODULES_INTERMEDIATES))) || exit "$$?"; \
				filtered_modules=$$(for n in $$all_modules; do \
					module_name=$$(basename $$n); \
					if [[ ! "$(SYSTEM_KERNEL_MODULES)" =~ "$$module_name" ]]; then echo $$n; fi; \
				done); \
				($(call build-image-kernel-modules-lineage,$$filtered_modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_KERNEL_MODULES_LOAD),,$(KERNEL_MODULES_INTERMEDIATES))) || exit "$$?"; \
				,\
				($(call build-image-kernel-modules-lineage,$$all_modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_KERNEL_MODULES_LOAD),,$(KERNEL_MODULES_INTERMEDIATES))) || exit "$$?"; \
			) \
