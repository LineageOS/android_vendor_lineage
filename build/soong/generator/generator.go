...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	cmd := variableSubstitution(ctx, String(g.properties.Cmd))
	...
	sandboxCommand := fmt.Sprintf("$sboxCmd --sandbox-path %s --output-root %s --copy-all-output -c %s",
		sandboxPath, genDir, rawCommand)
	...
	params := android.BuildParams{
		...
		Output:      android.PathForModuleGen(ctx, "dummy"), // just to keep ctx.Build() happy
		Implicits:   g.deps,
	}
