...
func (this *Mkdir) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	this.output = android.PathForModuleOut(ctx, "empty")

	builder := android.NewRuleBuilder(pctx, ctx)
	builder.Command().Textf("rm -rf %s && mkdir -p %s", installDir, installDir)
	builder.Command().
		Textf("rm -f %s && touch", this.output).
		Output(this.output)
	builder.Build("mkdir", fmt.Sprintf("mkdir %s", ctx.ModuleName()))
}
	...
	return []android.AndroidMkEntries{{
		...
		ExtraEntries: []android.AndroidMkExtraEntriesFunc{
			func(ctx android.AndroidMkExtraEntriesContext, entries *android.AndroidMkEntries) {
				entries.AddStrings("LOCAL_ADDITIONAL_DEPENDENCIES", this.output.String())
			},
		},
	}}
}
