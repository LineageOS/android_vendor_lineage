...
kernel_clang_version="$(clang_version "$clang_dir/bin/clang")"
...
for candidate in $(
    for dir in "$clang_root"/clang-*/; do
        [ -x "$dir/bin/clang" ] || continue
        version="$(clang_version "$dir/bin/clang")"
        [ -n "$version" ] || continue
        name="$(basename "${dir%/}")"
        revision="${name#clang-}"
        revision="${revision#r}"
    ...
    candidate_version="$(clang_version "${candidate%/lib}/bin/clang")"
