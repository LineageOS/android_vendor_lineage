...
# #include, so the probe neither depends on host headers nor needs bindgen to go
# looking for them.
...
probe() {
    ...
    # --formatter and --no-include-path-detection keep bindgen from reaching for
    # rustfmt, llvm-config and clang, which the build system's PATH sandbox
    # rightly complains about. --allowlist-type keeps the output to the one
