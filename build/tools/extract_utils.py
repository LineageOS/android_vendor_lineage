...
class CopyableBlob(Blob):
    def __init__(self, device, vendor, entry_line):
        ...
        # We can extract from system/vendor/... so don't branch
        self._extract_sources = [os.path.join('system', dst)]
        if dst is not src:
            self._extract_sources.append(os.path.join('system', src))
...
def extract(args, copyables, prebuilts):
    for copyable in copyables:
        copyable.extract(args)

    for prebuilt in prebuilts:
        prebuilt.extract(args)
...
if __name__ == '__main__':
    ...
    if args.extract:
        # Clean up proprietary directory if extracting
        shutil.rmtree(os.path.join(
            args.croot, 'vendor', args.vendor, args.device, 'proprietary'))

        set_up_adb()
        extract(args, copyables, prebuilts)
