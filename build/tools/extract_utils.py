...
class AdvHelpers:
    ...
    def write_product_copy_files(prop_list):
        ...
        for key, values in fin_files.items():
            with open(init_files[0], 'a') as target:
                target.write('    ' + f'vendor/{vendor}/{device}/proprietary/' + key + ':' + values + "\n")

        # Create dummy directories to copy files into
        for files in work_list:
            dir_path = f'{lineage_root}/vendor/{vendor}/{device}/proprietary/' + str(Path(files).parent)
    ...
    def write_product_packages(prop_list):
        ...
        for item in work_list:
            ...
            if '.apk' == suffix:
                ...
                module_format = f'Missing format for a specific extension?'

            dir_path = f'{lineage_root}/vendor/{vendor}/{device}/' + str(Path(src).parent)
            with open(init_files[1], 'a') as path:
                path.write('\n')
