...
def buildHeader(makefile, copyright):
  f = open(makefile, "w")

  current_year = datetime.datetime.now().year

  if current_year != copyright:
    f.write("# Copyright (C) " + copyright + "-" + str(current_year) + " The LineageOS Project\n")
  else:
  ...
  f.write("\n")
  f.write("# This file is generated by device/" + vendor + "/" + device + "/setup-makefiles.sh\n")
  f.write("\n")
...
def extract():
  ...
  with open(file, 'r') as f:
    for l in f:
      ...
      if section and section.lower() != current_section.lower():
        continue
      ...
      full_source = os.path.join(source, blob.source)
      if not os.path.isfile(full_source):
        full_source = os.path.join(source + "/vendor", blob.source)
      if not os.path.isfile(full_source):
        full_source = os.path.join(source + "/system", blob.source)
  ...
  for b in sorted(bloblist, key=operator.attrgetter('source')):
    ...
    if treble and b.target.split('/')[0] == "vendor":
      final_out = "$(TARGET_COPY_OUT_VENDOR)/" + b.target.replace("vendor/", "")
    else:
      final_out = "system/" + b.target

    mk.write(" \\\n    vendor/" + vendor + "/" + device + "/proprietary/" + b.target + ":" + final_out)
  ...
  for b in sorted(buildlist, key=operator.attrgetter('target')):
    ...
    amk.write("LOCAL_MODULE_OWNER := " + vendor + "\n")
    if multilib == "both":
