...
def fetch_query_via_ssh(remote_url, query):
    ...
    out = subprocess.check_output(f'ssh -x -p{port} {userhost} gerrit query --format=JSON --patch-sets --current-patch-set {query}', text=True)
    reviews = []
...
def fetch_query_via_http(remote_url, query, auth=True):
    ...
    url_suffix = f'/changes/?q={query}&o=CURRENT_REVISION&o=ALL_REVISIONS&o=ALL_COMMITS'
    if auth:
...
def main():
    ...
    if args.abandon_first:
        ...
        if needs_abandon:
            ...
            subprocess.run(f'repo abandon {args.start_branch[0]}')
            if not args.quiet:
                print('')
    ...
    for project_path, per_path_mergables in mergables.items():
        ...
        if args.start_branch:
            subprocess.run(f'repo start {args.start_branch[0]} {project_path}')
        ...
        branch_commits_count = int(subprocess.check_output(f'git rev-list --count {max_count} HEAD', cwd=project_path, text=True))
        ...
        for i in range(check_picked_count):
            if not commit_exists(project_path, 'HEAD~{0}'.format(i)):
                continue
            output = subprocess.check_output(f'git show -q HEAD~{i}', cwd=project_path, text=True)
            output = output.split()
...
def apply_change(args, item):
    ...
    if args.pull:
        ...
        if args.quiet:
            ...
            cmd_out = None
        result = subprocess.call(f'git cherry-pick --ff {item['revision']}', cwd=project_path, stdout=cmd_out, stderr=cmd_out)
        if result != 0:
