...
def fetch_query_via_ssh(remote_url, query):
    ...
    out = subprocess.check_output(
        f"ssh -x -p{port} {userhost} gerrit query --format=JSON --patch-sets --current-patch-set {query}",
        text=True,
    )
    reviews = []
...
def commit_exists(project_path, revision):
    return (
        subprocess.call(
            f"git cat-file -e {revision}", cwd=project_path, stderr=subprocess.DEVNULL
        )
        == 0
    )
    ...
    for project_path, per_path_mergables in mergables.items():
        ...
        branch_commits_count = int(
            subprocess.check_output(
                f"git rev-list --count {max_count} HEAD", cwd=project_path, text=True
            )
        )
        ...
        for i in range(check_picked_count):
            ...
            output = subprocess.check_output(
                f"git show -q HEAD~{i}", cwd=project_path, text=True
            )
            output = output.split()
...
def apply_change(args, item):
    ...
    if args.pull:
        ...
        result = subprocess.call(
            f"git cherry-pick --ff {item['revision']}",
            cwd=project_path,
            stdout=cmd_out,
            stderr=cmd_out,
        )
