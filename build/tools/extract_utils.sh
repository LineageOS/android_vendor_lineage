...
function write_product_packages() {
    ...
    local T_LIB32=( $(prefix_match_pattern "lib/*.so") )
    local T_LIB64=( $(prefix_match_pattern "lib64/*.so") )
    ...
    local T_HW_LIB32=( $(prefix_match_pattern "lib/hw/*.so") )
    local T_HW_LIB64=( $(prefix_match_pattern "lib64/hw/*.so") )
    local HW_MULTILIBS=( $(comm -12 <(printf '%s\n' "${T_HW_LIB32[@]}") <(printf '%s\n' "${T_HW_LIB64[@]}")) )
    local HW_LIB32=( $(comm -23 <(printf '%s\n'  "${T_HW_LIB32[@]}") <(printf '%s\n' "${HW_MULTILIBS[@]}")) )
    local HW_LIB64=( $(comm -23 <(printf '%s\n' "${T_HW_LIB64[@]}") <(printf '%s\n' "${HW_MULTILIBS[@]}")) )

    if [ "${#HW_MULTILIBS[@]}" -gt "0" ]; then
        write_packages "SHARED_LIBRARIES" "false" "both" "HW_MULTILIBS" "hw" >> "$ANDROIDMK"
    fi
    if [ "${#HW_LIB32[@]}" -gt "0" ]; then
        write_packages "SHARED_LIBRARIES" "false" "32" "HW_LIB32" "hw" >> "$ANDROIDMK"
    fi
    if [ "${#HW_LIB64[@]}" -gt "0" ]; then
        write_packages "SHARED_LIBRARIES" "false" "64" "HW_LIB64" "hw" >> "$ANDROIDMK"
    fi

    local T_V_LIB32=( $(prefix_match_pattern "vendor/lib/*.so") )
    local T_V_LIB64=( $(prefix_match_pattern "vendor/lib64/*.so") )
    ...
    local T_V_HW_LIB32=( $(prefix_match_pattern "vendor/lib/hw/*.so") )
    local T_V_HW_LIB64=( $(prefix_match_pattern "vendor/lib64/hw/*.so") )
    local V_HW_MULTILIBS=( $(comm -12 <(printf '%s\n' "${T_V_HW_LIB32[@]}") <(printf '%s\n' "${T_V_HW_LIB64[@]}")) )
    local V_HW_LIB32=( $(comm -23 <(printf '%s\n' "${T_V_HW_LIB32[@]}") <(printf '%s\n' "${V_HW_MULTILIBS[@]}")) )
    local V_HW_LIB64=( $(comm -23 <(printf '%s\n' "${T_V_HW_LIB64[@]}") <(printf '%s\n' "${V_HW_MULTILIBS[@]}")) )

    if [ "${#V_HW_MULTILIBS[@]}" -gt "0" ]; then
        write_packages "SHARED_LIBRARIES" "true" "both" "V_HW_MULTILIBS" "hw" >> "$ANDROIDMK"
    fi
    if [ "${#V_HW_LIB32[@]}" -gt "0" ]; then
        write_packages "SHARED_LIBRARIES" "true" "32" "V_HW_LIB32" "hw" >> "$ANDROIDMK"
    fi
    if [ "${#V_HW_LIB64[@]}" -gt "0" ]; then
        write_packages "SHARED_LIBRARIES" "true" "64" "V_HW_LIB64" "hw" >> "$ANDROIDMK"
    fi
