...
function write_product_packages() {
    ...
    if [ "${#P_MULTILIBS[@]}" -gt "0" ]; then
        write_blueprint_packages "SHARED_LIBRARIES" "odm" "both" "P_MULTILIBS" >> "$ANDROIDBP"
    fi
    if [ "${#P_LIB32[@]}" -gt "0" ]; then
        write_blueprint_packages "SHARED_LIBRARIES" "odm" "32" "P_LIB32" >> "$ANDROIDBP"
    fi
    if [ "${#P_LIB64[@]}" -gt "0" ]; then
        write_blueprint_packages "SHARED_LIBRARIES" "odm" "64" "P_LIB64" >> "$ANDROIDBP"
    fi
    ...
    if [ "${#P_APPS[@]}" -gt "0" ]; then
        write_makefile_packages "APPS" "odm" "" "P_APPS" >> "$ANDROIDMK"
    fi
    local P_PRIV_APPS=( $(prefix_match "odm/priv-app/") )
    if [ "${#P_PRIV_APPS[@]}" -gt "0" ]; then
        write_makefile_packages "APPS" "odm" "priv-app" "P_PRIV_APPS" >> "$ANDROIDMK"
    fi
    ...
    if [ "${#P_FRAMEWORK[@]}" -gt "0" ]; then
        write_makefile_packages "JAVA_LIBRARIES" "odm" "" "P_FRAMEWORK" >> "$ANDROIDMK"
    fi
    ...
    if [ "${#P_ETC[@]}" -gt "0" ]; then
        write_blueprint_packages "ETC" "odm" "" "P_ETC" >> "$ANDROIDBP"
    fi

    # Executables
    local BIN=( $(prefix_match "bin/") )
    if [ "${#BIN[@]}" -gt "0"  ]; then
        write_makefile_packages "EXECUTABLES" "" "" "BIN" >> "$ANDROIDMK"
    fi
    local V_BIN=( $(prefix_match "vendor/bin/") )
    if [ "${#V_BIN[@]}" -gt "0" ]; then
        write_makefile_packages "EXECUTABLES" "vendor" "" "V_BIN" >> "$ANDROIDMK"
    fi
    local P_BIN=( $(prefix_match "product/bin/") )
    if [ "${#P_BIN[@]}" -gt "0" ]; then
        write_makefile_packages "EXECUTABLES" "product" "" "P_BIN" >> "$ANDROIDMK"
    fi
    local P_BIN=( $(prefix_match "odm/bin/") )
    if [ "${#P_BIN[@]}" -gt "0" ]; then
        write_makefile_packages "EXECUTABLES" "odm" "" "P_BIN" >> "$ANDROIDMK"
    fi
