...
function build_kernel() {
    ...
    local KERNEL_BUILD_TOP="${ANDROID_BUILD_TOP}/../kernel/${lineage_version}/${target_kernel_source}"

    # Make sure we have the kernel source in place
    if [ ! -d "${KERNEL_BUILD_TOP}/.repo" ]; then
        echo "Kernel source ${KERNEL_BUILD_TOP} is missing, syncing"
        mkdir -p "${KERNEL_BUILD_TOP}"
        pushd "${KERNEL_BUILD_TOP}"
        ...
        repo init -u https://github.com/mikeNG/${target_kernel_manifest}.git ${repo_init_args}
        reposync
        popd
    fi
    ...
    # Build kernel
    pushd "${KERNEL_BUILD_TOP}"
    ./build_"${target_kernel_device}".sh
    popd

    # Remove previous kernel prebuilts
    if [ -d "$target_kernel_dir" ]; then
        find "${target_kernel_dir}" -maxdepth 1 ! \( -name .gitreview -o -name .gitignore \) -type f -exec rm -f {} +
    fi
    ...
    echo "Kernel build output copied to ${target_kernel_dir}/. Don't forget to breakfast your device again!"
}
